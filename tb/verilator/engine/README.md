...
| 8c..8g | 802.1AS-2011 10.3.10.2.1 qualifyAnnounce: a priority1-1 Announce from our own clockIdentity (a), with stepsRemoved 255 (b), or with our identity in its path trace (c: the second hop, the eighth hop at the bank's cap, the fourth of twelve beyond it) is refused ahead of every write: GM, parent, flags and the raw published vector hold, and the parser's drop counter does not move (a qualification refusal is not a header drop) |
| 8h | the boundary control adopts: stepsRemoved 254 and a one-hop path trace without us, landing in a bank whose upper hop words still hold our identity (the walk is gated by the TLV's hop count, not the bank's depth); the announcer's degrade hands mastership straight back (10.3.5) |
