...
| 16 | negative pdelay in [-80, 0) keeps asCapable (4.2.6.2.7) |
| 17 | over 800 ns clears it (neighborPropDelayThresh, 4.2.6.1.1) |
| 18 | recovery needs two good exchanges again |
...
All sixteen planted mutations (ladder trigger, both pdelay thresholds,
all three timeout values, a dead ratio divide, a deleted staleness
guard, the fall-at-three lost count, a dropped adopt-side sync void, a
dead step threshold, both servo sign errors, a dead integrator, a
halved integrator clamp, a step that loses the rate estimate) turn the
run red.
