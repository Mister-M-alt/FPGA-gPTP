...
rx_err, an unlisted messageType (Table 10-5 names Announce and
Signaling, Table 11-3 the five media-dependent types, and the NOTE under
Table 11-3 closes the set: the other nine of the sixteen values "are not
used in this standard", IEEE 1588-2008 Table 19 assigning three of them
to Delay_Req, Delay_Resp and Management and reserving the rest;
FPGA-gPTP #22): all nine, each in an otherwise valid 44-octet
frame every other arm admits, are refused at the type byte with no
event, no bank write and one counted drop, and the complete Pdelay_Req
after them still dispatches, and the Follow_Up shape (Table 11-9: 76 octets, the information
